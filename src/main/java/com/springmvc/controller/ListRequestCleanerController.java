...
@Controller
public class ListRequestCleanerController {
	@RequestMapping(value="/goListRequestCleaner",method=RequestMethod.GET)
	public String getRequestCleanerCategory(HttpSession session,HttpServletRequest request){
	
	
		Login user = (Login) session.getAttribute("user");		////ÃÑºÃËÑÊ¢Í§ login
			session.setAttribute("user", user);
			ListRequestCleanerManager lrcm = new ListRequestCleanerManager();
			List<Member> members  = lrcm.findCustomerDataForMember(user);	///¤é¹ËÒ¢éÍÁÙÅ¢Í§ÅÙ¡¤éÒ ¨Ò¡ÃËÑÊ¡ÒÃà¢éÒãªé§Ò¹
				int memid = 0 ;
				for(Member m : members) {
					memid = m.getMemberid();
				}
				List<Customer> customer = lrcm.findCustomerDataForCustomer(memid); ///Check data for Customer of Member
				...
				List<Request> requests = lrcm.queryListRequestCleaner(cusid);	///Check Request From Customer ID
			...
			//// Check Quotation No to Status02 
					List<Quotations> quotationsstatus2 = lrcm.getQuotationsID(listProivder); 
		...
		/// Check Quotation No to Status01 
				List<Quotations> quotations = lrcm.getQuotationsID(qid); ///¤é¹ËÒ¢éÍàÊ¹Í ¢Í§ÃÒÂ¡ÒÃ·Ó¤ÇÒÁÊÐÍÒ´·ÕèµéÍ§¡ÒÃ
				...
				List<ServicePerson> servicePersons = lrcm.getServicePersonID(cpid); /// ¤é¹ËÒ¢éÍÁÙÅ¢Í§¼ÙéãËéºÃÔ¡ÒÃ·Õè ÅÙ¡¤éÒ·Ó¡ÒÃÂ×¹ÂÑ¹àÊÃç¨ãºàÊ¹ÍÃÒ¤ÒáÅéÇ
				List<Company> companies	= lrcm.getCompanyID(cpid);
